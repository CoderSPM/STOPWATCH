...
class stopwatch(tk.Frame):
    ...
    def features(self):
        ...
        self.start_time_button=tk.Button(self,text="START",height=5,width=7,font=('arial',18,"bold"),background="green",command=self.start_time)
        self.start_time_button.pack(side=tk.LEFT)

        self.stop_time_button=tk.Button(self, text="STOP", height=5, width=7, font=('arial', 18, "bold"), background="red",command=self.pause_time)
        self.stop_time_button.pack(side=tk.LEFT)

        self.reset_time_button = tk.Button(self, text="RESET", height=5, width=7, font=('arial', 18, "bold"),background="yellow",command=self.reset_time)
        self.reset_time_button.pack(side=tk.LEFT)

        self.quit_button=tk.Button(self,text="QUIT",height=5, width=7, font=('arial', 18, "bold"),background="blue",command=self.window.quit)
    ...
    def start_time(self):
        if not self.running:
            ...
            self.change()
            self.running=True
            self.start_time_button['text'] = 'RESUME'
    ...
    def reset_time(self):
        ...
        self.start_time_button['text'] = 'START'
